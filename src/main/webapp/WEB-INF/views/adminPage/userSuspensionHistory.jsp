    ...
    <div id="wrapper1">
     	...
     	<div id = "paaaa">    	  
		   ...
		   <table id ="showlist">
		   	<thead>
		      <tr class="listhead">
		         <th class="nidx">정지회원 번호</th>
		         <th class="ntitle">조치 유형</th>
		         <th class="nid">정지 대상자</th>
		         <th class="ndate">정지 시작일</th>
		         <th class="nbhit">정지 종료일</th>
...
<script>
	...
	function drawList(list){
		...
		for(item of list){
		    console.log(item);
		    content += '<tr>';
		    content += '<td class="ntitle"><a href="userSuspensionDetail.go?banned_idx=' + item.banned_idx + '">' + item.banned_idx + '</a></td>'
		    content += '<td class="nidx">' + item.action_category + '</td>';
		    content += '<td class="nchb">' + item.user_name +'</td>';
		    ...
		    var stdate = new Date(item.report_date);
		    var enddate = new Date(item.report_date);
		    ...
		    content += '<td class="ndate">' + dateStr1 + '</td>';
		    content += '</tr>';
